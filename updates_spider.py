...
def get_updates(project_info):
    ...
    try:
        ...
        for page in range(0,5):
            cursor=str(page*20)
            cursor=str(base64.b64encode(cursor.encode('utf8')),encoding='utf8')
            json_data = [
                {
            ...
            # with open("updates.json", "w", encoding="utf-8") as fout:
            #     fout.write(response.text)
            #
            # with open("updates.json", "r", encoding="utf-8") as fin:
            ...
            if response.status_code==200:
                ...
                update_lists=project_data["timeline"]["edges"]
                ...
                for update_list in update_lists:
                    ...
                    updates_image_num=0
                    updates_video_num=0
                    updates_comments_num=0

                    if "data" in update_list["node"] and  update_list["node"]["data"] and "number" in update_list["node"]["data"]:
                        update_id = update_list["node"]["data"]["id"]
                        updates_count=update_list["node"]["data"]["number"]
                        updates_title=update_list["node"]["data"]["title"]
                        updates_creator=update_list["node"]["data"]["author"]["name"]
                        updates_datestamp=update_list["node"]["data"]["publishedAt"]
                        updates_date = datetime.datetime.fromtimestamp(updates_datestamp).strftime('%Y-%m-%d %H:%M:%S')

                        if "body" in update_list["node"]["data"] and update_list["node"]["data"]["body"]:
                            updates_content_html=update_list["node"]["data"]["body"]
                            if updates_content_html:
                                    updates_content_soup = BeautifulSoup(updates_content_html, 'html.parser')
                                    updates_content = updates_content_soup.get_text()

                                    updates_image_node=updates_content_soup.find_all("img")
                                    for updates_image_de in updates_image_node:
                                        if updates_image_de.has_attr('src'):
                                            updates_image_num+=1
                                            updateimagelist.append({"updates_count":updates_count,"row_num":updates_image_num,"image_url":updates_image_de["src"]})


                                    updates_video_node=updates_content_soup.find_all("iframe")
                                    for updates_video_de in updates_video_node:
                                        if updates_video_de.has_attr('src'):
                                            updates_video_num+=1
                                            updatevideolist.append({"updates_count":updates_count,"row_num":updates_video_num,"video_url":updates_video_de["src"]})

                                    vidionodes = updates_content_soup.find_all("video")
                                    for vidionode in vidionodes:
                                        videosource = vidionode.find("source")
                                        if videosource.has_attr('src'):
                                            video_url = videosource["src"]
                                            updates_video_num += 1
                                            updatevideolist.append({"updates_count": updates_count,"row_num": updates_video_num,"video_url": video_url})


                        updates_heart=update_list["node"]["data"]["likesCount"]
                        updates_comments_num=update_list["node"]["data"]["commentsCount"]

                        updateinfolist.append({
                            "updates_id":update_id,
                            "updates_count":updates_count,
                            "updates_title":updates_title,
                            "updates_creator":updates_creator,
                            "updates_date":updates_date,
                            "updates_content":updates_content,
                            # "updates_image_num":updates_image_num,
                            # "updates_video_num":updates_video_num,
                            # "updates_heart":updates_heart,
                            # "updates_comments_num":updates_comments_num
                        })

                if project_data["timeline"]["pageInfo"]["hasNextPage"] == False:
                    break


        resultdata["updateinfo"] = updateinfolist
        # resultdata["image_info"] = updateimagelist
        # resultdata["video_info"] = updatevideolist
...
def downloadupdateinfo():
    ...
    while True:
        ...
        begin_postion=0
        end_postion=1000000
        # end_postion=begin_postion

        querysql = """
               select a.pid,a.project_id, a.project_url  from crawling_url a where a.pid >=%s  AND a.pid<%s  and a.update_info=0   LIMIT 2000
            ...
            try:
                ...
                with ThreadPoolExecutor(max_workers=Max_works) as t1:
                    ...
                    for scrapy_url in scrapy_urls:
                        obj_list1.append(t1.submit(get_updates,scrapy_url))
                    ...
                    for future in as_completed(obj_list1):
                         ...
                         if retrunrewards["success"]==1:
                             try:
                                 ...
                                 if "updateinfo" in retrunrewards and   retrunrewards["updateinfo"]:
                                     updateinfos=retrunrewards["updateinfo"]

                                     insertupdateinfosql = """ insert into update_info(pid,project_id,project_link,updates_id,updates_count,
                                                                                   updates_title,updates_creator,updates_date,updates_content) 
                                                                               VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s) 
                                      """
                                     updateinfolist=[]
                                     for updateinfo in updateinfos:
                                         updateinfolist.append((retrunrewards["pid"],retrunrewards["project_id"],retrunrewards["project_link"],updateinfo["updates_id"],updateinfo["updates_count"],
                                                 updateinfo["updates_title"], updateinfo["updates_creator"],updateinfo["updates_date"], updateinfo["updates_content"]

                                                 ))

                                     cursor.executemany(insertupdateinfosql,updateinfolist)  # 执行SQL语句


                                 updatesql = """update crawling_url set update_info=1 where pid=%s """ %link_pid
